export const dict = {
  en: {
    hello: "Hello",
    world: "World",
    setup_welcome: "Welcome to the app",
    setup_get_started: "Get started",

    sidebar_button_overview: "Overview",
    sidebar_button_invoices: "Invoices",
    sidebar_button_clients: "Clients",
    sidebar_button_expenses: "Expenses",
    sidebar_button_reports: "Reports",
    sidebar_settings: "Settings",

    sidebar_section_sales: "Sales",
    sidebar_section_purchase: "Purchase",
    sidebar_section_reports: "Reports",
    sidebar_section_directory: "Directory",
  },
  cs: {
    hello: "Ahoj",
    world: "Světe",
    setup_welcome: "Vítejte v aplikaci",
    setup_get_started: "Začít",

    sidebar_button_overview: "Přehled",
    sidebar_button_invoices: "Faktury",
    sidebar_button_clients: "Klienti",
    sidebar_button_expenses: "Výdaje",
    sidebar_button_reports: "Reporty",
    sidebar_settings: "Nastavení",

    sidebar_section_sales: "Prodej",
    sidebar_section_purchase: "Nákup",
    sidebar_section_reports: "Reporty",
    sidebar_section_directory: "Adresář",
  },
};
